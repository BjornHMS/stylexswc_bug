import { BUTTON, BUTTON_PRIMARY, BUTTON_SECONDARY } from 'styles/themes/button.stylex';
import * as stylex from '@stylexjs/stylex';
import { useCallback, useState } from 'react';
import { B1, B2 } from 'styles/typography.stylex';

const s = stylex.create({
  button: (opt?: {
    theme: 'primary' | 'secondary';
    size: 'large' | 'xlarge';
  }) => ({
    justifyContent: 'center',
    alignContent: 'center',
    alignItems: 'center',
    display: 'grid',

    borderRadius: {
      xlarge: 16,
      large: 12,
    }[opt?.size ? opt.size : 'large'],

    padding: {
      xlarge: '28px 40px',
      large: '16px 40px',
    }[opt?.size ? opt.size : 'large'],

    color: {
      default: BUTTON.default_text,
      ':hover:not(:active)': BUTTON.hover_text,
      ':active': BUTTON.pressed_text,
    },

    borderWidth: 1,
    borderStyle: 'solid',
    borderColor: BUTTON.stroke,

    backgroundColor: {
      default: BUTTON.default,
      ':hover:not(:active)': BUTTON.hover,
      ':active': BUTTON.pressed,
    },
  }),
  list: {
    justifySelf: 'left',
    paddingLeft: 20,
  }
});

const buttonTheme = {
  primary: BUTTON_PRIMARY,
  secondary: BUTTON_SECONDARY,
}

const typography = {
  xlarge: B1,
  large: B2,
};

export const Button_Record_From_Import_Workaround: React.FC = () => {
  const [state, setState] = useState({
    count: 0,
    theme: 'primary' as keyof typeof buttonTheme,
    size: 'large' as keyof typeof typography,
  });

  const onClick = useCallback(() => {
    setState(prev => {
      const curr = prev.count < 3 ? prev.count + 1 : 0;
      let next = {...prev, count: curr};
      switch (curr) {
        case 0 : {
          next.theme = 'primary';
          next.size = 'large';
          break;
        }
        case 1 : {
          next.size = 'xlarge';
          break;
        }
        case 2 : {
          next.theme = 'secondary';
          next.size = 'large';
          break;
        }
        case 3 : {
          next.size = 'xlarge';
          break;
        }
      }
      return next;
    })
  }, []);

  return (
    <>
      <div {...stylex.props(s.list)}>
        This Component:
        <ul>
          <li>Creates a record with styles imported from a stylex.ts file.</li>
          <li>Sets a "valid" 1st param in stylex.props (undefined, false, anything crated with stylex.create)</li>
          <li>Uses ternary operator in styleX.create</li>
        </ul>
      </div>
      <button
        {...stylex.props(
          undefined, // <-- any valid stylex in 1st param works (undefined, false, anything crated with stylex.create)
          buttonTheme[state.theme],
          typography[state.size].Regular,
          s.button(state),
        )}
        onClick={onClick}
      >
        Click Me!
      </button>
    </>
  );
}